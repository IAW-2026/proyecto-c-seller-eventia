...
- **Eliminación de eventos con pedidos asociados:** un evento solo se puede eliminar si no tiene pedidos vinculados (un detalle es que si el evento tiene todos pedidos asociados en estado cancelado tampoco deja eliminarlo, se va a corregir para la 3era etapa). Si se permitiera borrarlo con pedidos existentes, quedarían pedidos sin `idEvento`, es decir, pedidos huérfanos sin sentido funcional. Además, ese cambio exigiría notificar y sincronizar a todas las web apps involucradas, algo que no se contempla en esta entrega. Esta restricción podría evaluarse recién en una tercera etapa.
